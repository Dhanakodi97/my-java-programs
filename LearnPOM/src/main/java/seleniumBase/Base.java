...
public class Base implements SelenBase {
	long timeOuts = 30;
	int maxTimeWait = 10;
	...
	public void setUp(String url) {
		...
		driver.get(url);
		//wait = new WebDriverWait(driver, maxTimeWait	);
		wait = new WebDriverWait(driver, Duration.ofSeconds(maxTimeWait));
	}
	...
	public void setUp(Browser browser, String url) {
		...
		default:
			System.err.println("This driver is not defined");
			break;
		}
		driver.manage().window().maximize();
		driver.manage().timeouts().implicitlyWait(timeOuts, TimeUnit.SECONDS);
	...
	public void typeAndEnter(WebElement ele, String testData) {
		WebElement element = wait.until(ExpectedConditions.visibilityOf(ele));
		element.clear();
		element.sendKeys(testData);
	}
	...
	@Override
	public void type(WebElement ele) {
